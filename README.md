...
**Note:** Manual recently updated to include a feature to prevent the player from clicking out-of-logic checks, and this is enabled by default. Because some of the logic in this game is based around not requiring the player to do checks that would require incredibly good rng, you will probably be able to get at least a few checks out of logic every game, so you may need to turn this setting off by entering the command /open_settings in the manual client and turning the option "Stop accidental button press" OFF.
